 ...
 *
 * @param {string}  handler              Stripe handler to call - either `handleCardPayment` or `handleCardSetup`.
...
jQuery( function( $ ) {
	...
	var RCP_Stripe_Registration = {
		...
		handlePayment: function ( event, form, response ) {
			...
			let handler = 'payment_intent' === response.gateway.data.stripe_intent_type ? 'handleCardPayment' : 'handleCardSetup';
			let args = {
				payment_method_data: {
