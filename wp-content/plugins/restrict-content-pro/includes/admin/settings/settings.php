				...
				<div class="tab_content" id="general">
					<table class="form-table">
						...
						<tr valign="top" style="display: none">
							<th>
								<label style="visibility: hidden;" for="rcp_settings[license_key]"><?php _e( 'License Key', 'rcp' ); ?></label>
							</th>
							<td>
								<input type="hidden" class="regular-text" id="rcp_settings[license_key]" style="width: 300px;" name="rcp_settings[license_key]" value="<?php if( isset( $rcp_options['license_key'] ) ) { echo $rcp_options['license_key']; } ?>"/>
								<?php $status = get_option( 'rcp_license_status' ); ?>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									<?php wp_nonce_field( 'rcp_deactivate_license', 'rcp_deactivate_license' ); ?>
									<input type="hidden" class="button-secondary" name="rcp_license_deactivate" value="<?php _e('Deactivate License', 'rcp'); ?>"/>
									<span style="color:green;"><?php _e('active', 'rcp' ); ?></span>
								<?php } elseif( $status !== 'valid' ) { ?>
									<input type="hidden" class="button-secondary" name="rcp_license_activate" value="<?php _e('Activate License', 'rcp' ); ?>"/>
								<?php } ?>
								<p class="description" style="visibility: hidden"><?php printf( __( 'Enter license key for Restrict Content Pro. This is required for automatic updates and <a href="%s">support</a>.', 'rcp' ), 'http://restrictcontentpro.com/support' ); ?></p>
							</td>
						</tr>
...
function rcp_sanitize_settings( $data ) {
	...
	// TODO Remove License checks because field is no longer required
	...
	if( ! empty( $_POST['rcp_license_deactivate'] ) ) {
		rcp_log( sprintf('RCP Deactivate License Key called with License Key: %s', $data['license_key'] ) );
	} elseif( ! empty( $data['license_key'] ) ) {
		rcp_log( sprintf('RCP Activate License Key called with License Key: %s', $data['license_key'] ) );
	}
...
function rcp_activate_license() {
	// TODO Remove function entirely because it is no longer required
...
// TODO remove this check license because it is no longer required
